...
def write_accel_io(y: List[List[float]]):
    from fft import SCALE, MAX_N_PER_FFT, FFT_TWIDDLES, NUM_FFT_TWIDDLES, REAL_FFT_TWIDDLES, NUM_REAL_FFT_TWIDDLES, fft
    ...
    values.append(NUM_FFT_TWIDDLES)

    for twiddle in FFT_TWIDDLES:
...
def write_accel_io(y: List[List[float]]):
    from fft import (SCALE, MAX_N_PER_FFT, FFT_TWIDDLES, NUM_FFT_TWIDDLES, REAL_FFT_TWIDDLES, NUM_REAL_FFT_TWIDDLES, real_fft)
    ...
    assert all(-2**(bits_per_value-1) <= v < 2**(bits_per_value-1) for v in quant_y), \
        "Quantized values exceed the representable range"

    # Flash format:
    # [n, chunks, num_of_fft_twiddles,
    #  fft_tw0_re, fft_tw0_im, ...,
    #  unpack_tw0_re, unpack_tw0_im, ...,
    #  audio_data...]
    values = [MAX_N_PER_FFT * chunks, chunks]

    # Number of FFT twiddles
    values.append(NUM_FFT_TWIDDLES)

    # FFT twiddles (interleaved re/im)
    for tw in FFT_TWIDDLES:
        values.append(int(tw.real))
        values.append(int(tw.imag))

    # Unpack twiddles (interleaved re/im)
    for tw in REAL_FFT_TWIDDLES:
        values.append(int(tw.real))
        values.append(int(tw.imag))
    ...
    # Golden reference using real_fft
    ground_truth = []
    for quant_y_chunk in chunk_list(quant_y, MAX_N_PER_FFT):
        fft_result = real_fft(quant_y_chunk)
        ground_truth.extend(fft_result)
